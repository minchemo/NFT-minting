...
export default function () {
    ...
    const getAccountBalance = () => {
        ...
        store.state.web3.eth.getBalance(store.state.connectedAddress, (err, balance) => {
    ...
    const publicSaleMint = (amount) => {
        ...
        if (store.state.nftConfig.publicSalePrice * amount >= store.state.accountBalance) {
            alert('錢包餘額不足，請至少準備 (購買金額 + Gas 費用)')
        } else {
            return store.state.web3.eth.sendTransaction(transactionParams, (err, hash) => {
    ...
    const preSaleMint = (amount) => {
        ...
        if (store.state.nftConfig.preSalePrice * amount >= store.state.accountBalance) {
            alert('錢包餘額不足，請至少準備 (購買金額 + Gas 費用)')
        } else {
            return store.state.web3.eth.sendTransaction(transactionParams, (err, hash) => {
                const interval = setInterval(function () {
