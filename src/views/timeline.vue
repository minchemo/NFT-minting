<template>
  <div class="container mx-auto h-full flex flex-col pb-40">
    <PageTitle title="timeline タイムライン
    ...
    <div class="container mx-auto p-8 pt-0 pb-20 h-full scrollbar">
      <div class="flex mb-8 font-['Nunito'] gap-8">
        <div class="flex items-center">
          <span class="relative inline-flex rounded-full h-3 w-3 bg-green-500 mr-4"></span>
          <p class="uppercase font-black">finished</p>
        </div>
        <div class="flex items-center">
          <span class="relative inline-flex rounded-full h-3 w-3 bg-orange-500 mr-4"></span>
          <p class="uppercase font-black">processing</p>
        </div>
        <div class="flex items-center">
          <span class="relative inline-flex rounded-full h-3 w-3 bg-gray-500 mr-4"></span>
      ...
      <div class="grid grid-rows-2 grid-cols-4 gap-5">
        ...
        <template v-for="(item, i) in timelines">
          <div data-aos="zoom-in" :data-aos-delay="i * 100"
            ...
            <div class="text-sm tracking-wide leading-relaxed transition-all text-cyan-800" v-html="item.description">
            </div>
            <div class="absolute flex items-center left-8 bottom-8">
              <span class="flex h-3 w-3 mr-3">
                <span class="absolute inline-flex h-3 w-3 rounded-full opacity-75" v-bind:class="{
            ...
            <div class="absolute w-1/2 aspect-square bg-contain right-0 bottom-0"
              v-bind:style="{ backgroundImage: `url('${item.img}')` }"></div>
          </div>
        </template>
      </div>
    </div>
