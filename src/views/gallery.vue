<template>
  <div class="fixed top-0 left-0 w-screen h-1 bg-white z-50">
    <div class="absolute h-full top-0 left-0 bg-cyan-800 transition-all" v-bind:style="{ 'width': `${progress}%` }"></div>
  </div>
  <div class="container mx-auto h-full flex flex-col pb-40">
    <PageTitle title="gallery ギャラリー" subtitle="Jidori artworks" />

    <div class="flex px-8 pb-8 items-center justify-between">
      <div class="w-full flex gap-8">
        ...
        <div class="w-1/6">
          <div @click="isGirl = !isGirl" v-bind:class="{
            'bg-white': isGirl,
            'text-black': isGirl,
            'border-2': isGirl,
            'bg-black': !isGirl,
      ...
      <div class="font-black font-['Open_sans'] text-5xl text-gray-300 whitespace-nowrap">
        3950
      </div>
    </div>

    <div class=" container mx-auto grid grid-cols-7 gap-6 px-8 pb-8" :key="reloadTs" ref="gallery">
      <template v-for="i in ids.slice(0, items)">
        <div>
          <div
            ...
            :data-bg="`https://jidori-nft.com/tokens/${i - 1}.jpg`"></div>
          <p class="font-['Nunito'] uppercase text-sm text-gray-500 tracking-widest text-center pt-2 font-black">
            Jidori
          </p>
          <p class="font-['Nunito'] uppercase text-sm text-gray-500 tracking-widest text-center font-thin">
            # {{ i - 1 }}
          </p>
        </div>
      </template>
